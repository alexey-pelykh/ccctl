// SPDX-License-Identifier: AGPL-3.0-only
// Copyright (C) 2026 Oleksii PELYKH

/**
 * Session-create response wire DTO — the server HTTP boundary's single, testable
 * seam between `@ccctl/core`'s internal model and the bytes on the foreign
 * transport.
 *
 * `@ccctl/core`'s {@link SessionCreateResponse} stays idiomatic camelCase
 * (`{ sessionId, wsUrl }`) — the hub model is not bent to a foreign wire. The
 * response body, however, is **snake_case** (`{ session_id, ws_url }`): it is
 * exchanged with Claude Code's own Agent-SDK `stream-json` (`--sdk-url`) control
 * transport, whose convention is snake_case and whose session identifier is
 * universally `session_id` — decided on primary-source SDK/CLI evidence in
 * ADR-001. The SAME `{ session_id, ws_url }` body is returned by the current flow's
 * session-create (§2, `POST /v1/sessions`) and by the retained legacy register
 * route (`POST /v1/code/sessions`), so one mapper serves both.
 *
 * This module is the ONE place that crosses that camel↔snake boundary.
 * {@link toRegisterResponseWire} is the mapper; a golden/contract test pins the
 * exact serialized bytes so any drift (a renamed key, a reverted casing, a stray
 * field) fails closed in CI (bridge-protocol §1/§2). Keeping the whole concern here
 * means a future wire change is one edit in this mapper plus its golden — core
 * and its internal consumers do not move.
 *
 * The `toRegisterResponseWire` / `RegisterResponseWire` names are retained (over
 * the more literal `toSessionCreateResponseWire`) because the not-yet-realigned
 * `@ccctl/e2e` harness imports them by name; they are renamed when the e2e realigns
 * onto the current flow (#124).
 *
 * @see docs/decisions/adr-001-register-response-wire-casing.md
 */

import type { SessionCreateResponse } from "@ccctl/core";

/**
 * The exact on-the-wire shape of the session-create / register response body —
 * snake_case per ADR-001, deliberately asymmetric with core's camelCase
 * {@link SessionCreateResponse}. The field order here is the serialized key order
 * the golden test pins.
 */
export interface RegisterResponseWire {
  /** Server-assigned session identifier (maps from {@link SessionCreateResponse.sessionId}). */
  readonly session_id: string;
  /** The worker-channel WebSocket URL (maps from {@link SessionCreateResponse.wsUrl}). */
  readonly ws_url: string;
}

/**
 * Serialize core's camelCase {@link SessionCreateResponse} into the snake_case
 * {@link RegisterResponseWire} body. The explicit boundary seam ADR-001 requires:
 * the session-create / register handler writes `toRegisterResponseWire(response)`
 * rather than the core object directly, so the wire casing is a deliberate,
 * golden-tested mapping and never an accident of core's internal field names.
 */
export function toRegisterResponseWire(response: SessionCreateResponse): RegisterResponseWire {
  return {
    session_id: response.sessionId,
    ws_url: response.wsUrl,
  };
}
